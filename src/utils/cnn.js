    ...
    for (let i = 0; i < layer.num_neurons; i++) {
      ...
      if (curLayerType === nodeType.CONV || curLayerType === nodeType.FC) {
        ...
        let preNodeWidth = nn[curLayerIndex - 1][0].output.length,
          ...
          link = new Link(nn[curLayerIndex - 1][preNodeIndex], node, [
            preNodeRow,
            preNodeCol,
          ]);
    ...
    fetch("/assets/data/nn_10.json")
      .then((response) => {
        response.json().then((nnJSON) => {
          getInputImageArray(inputImageFile).then((inputImageArray) => {
...
export const singleConv = (input, kernel, stride = 1, padding = 0) => {
  ...
  for (let r = 0; r < stepSize; r++) {
    for (let c = 0; c < stepSize; c++) {
      let curWindow = matrixSlice(
        input,
        r * stride,
        r * stride + kernel.length,
        c * stride,
        c * stride + kernel.length
      );
  ...
  for (let r = 0; r < stepSize; r++) {
    for (let c = 0; c < stepSize; c++) {
      let curWindow = matrixSlice(
        mat,
        r * stride,
        r * stride + kernelWidth,
        c * stride,
        c * stride + kernelWidth
      );
